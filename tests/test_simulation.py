...
def test_simulation_initialization(empty_simulation):
    assert len(empty_simulation.bodies) == 0

def test_add_default_body(empty_simulation):
...
def test_unique_name_generation(empty_simulation):
    b1 = empty_simulation.add_default_body(0.0, 0.0)
    b2 = empty_simulation.add_default_body(10.0, 10.0)
    
    assert b1.name != b2.name

def test_remove_body_by_index(empty_simulation):
...
def test_remove_invalid_index(empty_simulation):
    empty_simulation.add_default_body(0.0, 0.0)
    # Не должно падать с IndexError
    empty_simulation.remove_body_by_index(999) 
    assert len(empty_simulation.bodies) == 1

def test_simulation_step_empty(empty_simulation):
